...
some modifications and clean-up by Wolfgang, April 2016.
...
#             if time.time() - float(time_str)*1e-3 > 3600*24:
#                 marked_for_death = True
...
class Dot_AWG_Load():
    def __init__(self, seqs, path=None, awg=None, awg_dir=None):
        ...
        self.awg_path = os.path.join(awg_dir, os.path.split(self.path)[-1])
...
#        logging.info('awg_files: completed move from %s to %s' % (src, destination))
    ...
    def load_seqs(self, delay_override=120e3, verbose=False):
        MAX_ATTEMPTS = 6
        '''The main routine'''
#        self.parse_seqs()
        # generate sequence only once
        if self.load_attempt_no == 0:
            self.parse_seqs()
...
#            time.sleep(10)
            logging.info('awg_files - %s: finished clear_awg_and_pull_data()' % (self.awg.get_name()))
...
#            self.path = self.local_path
            ...
            self.awg.load_dot_awg(self.awg_path, timeout=120e3)
            if delay_override:
        ...
        #     raise
            ...
            if self.load_attempt_no < MAX_ATTEMPTS:
                #self.awg.get_id(timeout=delay_override)
                msg = 'Failed AWG load, big loop, retrying. (%d/%d) [%s]' % (self.load_attempt_no, MAX_ATTEMPTS, e)
                logging.info(msg)
                logging.info(traceback.format_exc(e))
                self.awg.wait_done(timeout=delay_override)
                self.load_seqs(delay_override=delay_override, #timeout=delay_override,
                               verbose=verbose)
            else:
    ...
    def clear_awg_and_pull_data(self, delay):
        ...
        self.awg.wait_done(timeout=delay)
        self.awg.pull_dot_awg(self.awg_path)
        self.awg.wait_done()
    ...
    def parse_seqs(self):
        ...
        ch = 0
        self.rl = [] # reset rl everytime we try to load seqs (150519)
        for ch in self.seqs:
            if type(ch) != int:
                continue
    ...
    def add_waveform(self, idx, chan):
        ...
        m1_chan = '%dm1' % chan
        if self.seqs.has_key(m1_chan) and self.seqs[m1_chan] is not None:
            m1 = self.seqs[m1_chan][idx]
            m1_name = m1.get_name()
            m1_data = m1.get_data()
        else:
            m1_name = 'delay%d' % pulse_len
            m1_data = None # np.zeros(pulse_len)

        m2_chan = '%dm2' % chan
        if self.seqs.has_key(m2_chan) and self.seqs[m2_chan] is not None:
            m2 = self.seqs[m2_chan][idx]
            m2_name = m2.get_name()
            m2_data = m2.get_data()
        else:
            m2_name = 'delay%d' % pulse_len
            m2_data = None # np.zeros(pulse_len)
    ...
    def write_dot_awg(self, write_timeout=120): # seconds
        recs = [' ']*len(self.rl)
        ...
        flush_freq = 100
        flush_period = max(1, len(recs) / flush_freq)
        ...
        logging.info('.AWG file final: %d bytes (%0.2f seconds)' %
                    (final_file_size, (time.time() - time_start)))
        return

        # see if the file is done writing
#        while (time.time() < (time_start + write_timeout)): # in seconds
#            if f.closed:
#                final_file_size = os.path.getsize(self.path)
#                logging.info('.AWG file final: %d bytes (%0.2f seconds)' %
#                            (final_file_size, (time.time() - time_start)))
#                logging.info('%0.2f' % (time.time() - time_start))
#                return
#            time.sleep(0.1)
